...
- Track：再连续 in-zone 满 ``track_horizon_s`` → success 终止。
...
class FormationEnv:
    ...
    def __post_init__(self) -> None:
        ...
        self.observation_spec = ObservationSpec.from_config(self.cfg)
        self.tugs = [TugboatDynamicsModel() for _ in range(self.n_tugs)]
        self.ship = LargeShipModel(
            length_m=self.cfg.ship_length_m, beam_m=self.cfg.ship_beam_m,
            slot_lon_offset_m=self.cfg.slot_lon_offset_m, slot_lat_offset_m=self.cfg.slot_lat_offset_m,
            speed_min=self.cfg.ship_speed_min, speed_max=self.cfg.ship_speed_max,
            yaw_rate_max=self.cfg.ship_yaw_rate_max,
            speed_tau=self.cfg.ship_speed_tau_s, yaw_tau=self.cfg.ship_yaw_tau_s,
            target_resample_min_s=self.cfg.ship_target_resample_min_s, target_resample_max_s=self.cfg.ship_target_resample_max_s,
            rng=self.rng,
        )
        self.step_count = 0
        self.last_actions = np.zeros((self.n_tugs, ACTION_DIM), dtype=np.float32)
        hist_len = self.observation_spec.history_len
        ...
        self._episode = MutableEpisodeState(
            ...
            just_captured=False,
            phase="approach",
            track_streak_steps=0,
            track_steps_total=0,
            track_steps_all_in_zone=0,
        )
    ...
    def obs_dim(self) -> int:
        return self.observation_spec.total_dim
    ...
    def reset(self, seed: int | None = None) -> np.ndarray:
        ...
        self._episode.track_streak_steps = 0
        self._episode.track_steps_total = 0
    ...
    def step(self, actions: np.ndarray) -> tuple[np.ndarray, np.ndarray, np.ndarray, dict]:
        ...
        # 捕获奖励：刚进入 Track 时发一次；最终 success 不再重复发放
        if term_info.get("just_captured") and self.cfg.reward_arrival_bonus > 0.0:
    ...
    def _check_termination(self, slot_world: np.ndarray) -> tuple[np.ndarray, dict[str, Any]]:
        ...
        cfg = self.cfg
        ep = self._episode
        ep.just_captured = False

        hold_steps = max(1, int(round(cfg.hold_time_s / cfg.dt_ctrl)))
        track_steps = max(1, int(round(float(getattr(cfg, "track_horizon_s", 30.0)) / cfg.dt_ctrl)))
        all_in_zone = all(int(self.in_zone_steps[i]) >= 1 for i in range(n))
        all_held = all(int(self.in_zone_steps[i]) >= hold_steps for i in range(n))

        # 更新 Capture / Track 进度（碰撞检查前先更新，便于 info 一致）
        if not ep.capture_done and all_held:
            ep.capture_done = True
            ep.just_captured = True
            ep.phase = "track"
            ep.track_streak_steps = 0
            ep.track_steps_total = 0
            ep.track_steps_all_in_zone = 0

        # track_horizon 从 Capture 之后的下一步开始累计
        if ep.capture_done:
            ep.phase = "track"
            if not ep.just_captured:
                ep.track_steps_total += 1
                if all_in_zone:
                    ep.track_streak_steps += 1
                    ep.track_steps_all_in_zone += 1
                else:
                    ep.track_streak_steps = 0

        track_ratio = (
            float(ep.track_steps_all_in_zone) / float(ep.track_steps_total)
            if ep.track_steps_total > 0
            else 0.0
        )
        info: dict[str, Any] = {
            "success": False,
            "capture": bool(ep.capture_done),
            "just_captured": bool(ep.just_captured),
            "phase": str(ep.phase),
            "track_streak_steps": int(ep.track_streak_steps),
            "track_steps_total": int(ep.track_steps_total),
            "track_in_zone_ratio": float(track_ratio),
            "collision": False,
            "timeout": False,
            "terminated": False,
            "truncated": False,
        ...
        # Track 成功：捕获后连续全体 in-zone 满 track_horizon
        if ep.capture_done and ep.track_streak_steps >= track_steps:
            dones[:] = True
            info["success"] = True
            info["terminated"] = True
            return dones, info
    ...
    def render_snapshot(self) -> dict[str, Any]:
        ...
        for i, tug in enumerate(self.tugs):
            ...
            tugs_data.append({
                ...
                "r_dist": float(self.last_reward_components.get("r_dist", np.zeros(self.n_tugs))[i]),
                "r_hold": float(self.last_reward_components.get("r_hold", np.zeros(self.n_tugs))[i]),
