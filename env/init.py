...
from env.observer import ACTION_DIM, local_to_world, world_to_local
...
class InitSampler:
    ...
    def _sample_ship_tracking_motion(
        ...
        *,
        ...
        env = self._env
        speed_offset = float(env.rng.uniform(speed_offset_min, speed_offset_max))
        sway = float(env.rng.uniform(-sway_noise_ms, sway_noise_ms))
        ship_body_vx = env.ship.u + speed_offset
        ship_body_vy = env.ship.v + sway
        ...
        if position_body is not None and target_body is not None:
            ...
            if dist > 1e-6:
                ...
                approach_speed = float(env.rng.uniform(lo, hi))
                ship_body_vx = env.ship.u + approach_speed * dx / dist
                ship_body_vy = env.ship.v + approach_speed * dy / dist + sway

        vx_w, vy_w = local_to_world(ship_body_vx, ship_body_vy, env.ship.psi)
        if math.hypot(vx_w, vy_w) > 1e-4:
            heading_base = math.atan2(vy_w, vx_w)
        else:
            heading_base = env.ship.psi
        psi = _wrap_pi(
            heading_base + float(env.rng.uniform(-heading_noise_rad, heading_noise_rad))
        )
        u_tug, v_tug = world_to_local(vx_w, vy_w, psi)
        r_tug = float(env.rng.uniform(-yaw_rate_noise_rads, yaw_rate_noise_rads))

        action = np.zeros(ACTION_DIM, dtype=np.float32)
        forward = float(
            np.clip(
                forward_action + env.rng.uniform(-forward_action_jitter, forward_action_jitter),
                -1.0,
                1.0,
            )
        )
    ...
    def _init_position_is_safe(
        ...
        if self._env.ship.distance_from_hull(x_w, y_w) < min_hull_dist:
            return False
        for px, py in placed:
    ...
    def _sample_ready_slot_state(
        ...
        env = self._env
        cfg = env.cfg
        slot_body = env.ship.slot_positions_body()[int(slot_idx), :2]
        ...
        for _ in range(120):
            x_b = float(slot_body[0] + env.rng.uniform(-jitter, jitter))
            y_b = float(slot_body[1] + side * outward + env.rng.uniform(-jitter, jitter))
            x_w, y_w = env._ship_body_to_world_xy(x_b, y_b)
        ...
        if chosen_xy is None:
            ...
            chosen_xy = env._ship_body_to_world_xy(x_b, y_b)
    ...
    def _sample_mixed_route_body_candidate(
        ...
        cfg = env.cfg
        side = self._slot_side_sign(slot_idx)
        slot_body_arr = env.ship.slot_positions_body()[int(slot_idx), :2]
        slot_body = (float(slot_body_arr[0]), float(slot_body_arr[1]))
        l_half = env.ship.length_m / 2.0
        ...
        if zone == "rear_lane":
            ...
            x_b = float(
                -l_half
                - env.rng.uniform(dist_min, dist_max)
                + env.rng.uniform(-lon_jitter, lon_jitter)
            )
            y_b = float(side * lane_abs + env.rng.uniform(-lat_jitter, lat_jitter))
            ...
            x_b = float(
                -l_half
                - env.rng.uniform(gate_lo, gate_hi)
                + env.rng.uniform(-0.5 * lon_jitter, 0.5 * lon_jitter)
            )
            y_b = float(side * lane_abs + env.rng.uniform(-0.6 * lat_jitter, 0.6 * lat_jitter))
            target = slot_body
        elif zone == "side_lane":
            side_lat = max(
                lane_abs,
            ...
            x_b = float(
                env.rng.uniform(x_lo, x_hi)
                + env.rng.uniform(-0.3 * lon_jitter, 0.3 * lon_jitter)
            )
            y_b = float(side * side_lat + env.rng.uniform(-0.6 * lat_jitter, 0.6 * lat_jitter))
            target = slot_body
            ...
            x_b = float(slot_body[0] + env.rng.uniform(-x_span, x_span))
            y_b = float(
                slot_body[1]
                + side * env.rng.uniform(outward_min, outward_max)
                + env.rng.uniform(-0.5 * lat_jitter, 0.5 * lat_jitter)
            )
            target = slot_body
            ...
            x_b = float(
                -l_half
                - env.rng.uniform(dist_lo, dist_hi)
                + env.rng.uniform(-lon_jitter, lon_jitter)
            )
            y_b = float(-side * opposite_lane_abs + env.rng.uniform(-lat_jitter, lat_jitter))
            target = (-l_half - gate_dist, side * lane_abs)
        else:
        ...
        *,
        ...
        for fallback_side in fallback_sides:
            ...
            target = (
                ...
                else tuple(float(v) for v in env.ship.slot_positions_body()[int(slot_idx), :2])
            )
            for k in range(48):
    ...
    def sample_mixed_slot_approach_states(
        ...
        ready_count = int(env.rng.choice(np.asarray(ready_counts, dtype=np.int32)))
        ready_slots = set(int(v) for v in env.rng.choice(n, size=ready_count, replace=False))
        env._mixed_ready_tugs = ready_slots
