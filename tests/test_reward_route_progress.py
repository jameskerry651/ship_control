...
def test_nonfinal_route_progress_controls_route_reward() -> None:
    ...
    for i, tug in enumerate(env.tugs):
        route_len = len(env._route_waypoints_body_for_tug(i))
    ...
    route_remaining = env._route_remaining_distance(tug_idx)
    clip = float(env.cfg.reward_target_progress_clip_m)

    env._episode.prev_route_remaining[tug_idx] = route_remaining + clip
    _, info_forward = env._compute_rewards(actions)

    env._episode.in_zone_steps[:] = 0
    env._episode.prev_route_remaining[tug_idx] = route_remaining - clip
    _, info_backward = env._compute_rewards(actions)
...
def test_prev_route_remaining_tracks_current_route_after_step() -> None:
    ...
    current = np.asarray(
        [env._route_remaining_distance(i) for i in range(env.cfg.n_tugs)],
        dtype=np.float32,
    )
    np.testing.assert_allclose(env._episode.prev_route_remaining, current, rtol=1e-5, atol=1e-5)
